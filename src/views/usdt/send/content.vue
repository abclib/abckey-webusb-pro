      ...
      <div class="form">
        <div class="d-flex flex-column justify-start align-start pl-2">
          ...
          <div class="subtitle-2" :class="eth.balance.toString(10) !== '0' ? 'primary--text' : 'red--text'">
            USDT {{ $t('Available Balance') }}：{{ UnitHelper(eth.balance).div(1000000).toString(10) }}
            USDT
          </div>
...
export default {
  ...
  methods: {
    ...
    async getUtxoList() {
      ...
      const result = await HTTP.Transaction.BalanceByAddress({ coinName: 'eth', address: address })
      if (result.balance) {
